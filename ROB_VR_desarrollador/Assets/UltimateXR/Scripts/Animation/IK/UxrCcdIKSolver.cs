...
namespace UltimateXR.Animation.IK
{
    ...
    public partial class UxrCcdIKSolver : UxrIKSolver
    {
        ...
        private Vector3 _storedGoalPosition;
        ...
        {
            //_goal = pieza3.transform;

            // _links[1].InitialLocalRotation= pieza1.transform.rotation;
            //_links[2].InitialLocalRotation= pieza2.transform.rotation;
            //_links[3].InitialLocalRotation= pieza3.transform.rotation;


            //for (int i = 0; i < _links.Count; i = i + 1)
            //{
             //   Debug.Log("Valor i;" + i);
             //       _links[i+1].InitialLocalRotation = _links[i+1].Bone.transform.rotation;
           ...
           // }
            ...
            for (int i = 0; i < _links.Count - 1; i++)
            {
                ...
                string piezaName = "pieza" + (i + 1);

                

                // Buscar el objeto correspondiente en la jerarquía de Unity
                GameObject pieza = GameObject.Find(piezaName);

                // Verificar si se encontró el objeto y si tiene una referencia válida al componente Transform
                if (_links[i] != null && _links[i].Bone.transform != null)
                {
                    // Asignar la rotación inicial del enlace a la rotación actual del objeto
                    _links[i].InitialLocalRotation = pieza.transform.rotation;
                    //_links[i].InitialLocalRotation = _links[i].Bone.transform.rotation;
                }
                else
                {
                    // Manejar la situación donde el objeto correspondiente no se encuentra
                    Debug.LogWarning("No se pudo encontrar el objeto correspondiente para el enlace " + i);
                }
            }
