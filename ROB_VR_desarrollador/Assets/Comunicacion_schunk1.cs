...
public class Comunicacion_schunk1 : MonoBehaviour
{
    ...
    public IEnumerator SendSelectedTrajectory()
    {
        ...
        foreach (float[] punto in puntos)
        {
            ...
            yield return new WaitForSeconds(tiempo_envio / 3);
        }
    ...
    public void WritePoint()
    {
        ...
        {
            File.Create(filePoint).Close();
        }

        // Escribir los datos en el archivo de texto
        using (StreamWriter writer = new StreamWriter(filePoint))
        {
        ...
        Debug.Log("Datos guardados en el archivo: " + filePoint);
    }
    ...
    public void ReadPoint()
    {
        ...
        if (!File.Exists(filePoint))
        {
            Debug.LogError("El archivo no existe: " + filePoint);
            return;
        }
        ...
        string[] lines = File.ReadAllLines(filePoint);
        ...
        Debug.Log("Datos leídos del archivo: " + filePoint);
    }
    ...
    public void WriteTrajectory()
    {
        ...
        if (!File.Exists(fileTrayectoria))
        {
            File.Create(fileTrayectoria).Close();
        }


        using (StreamWriter writer = new StreamWriter(fileTrayectoria))
        {
    ...
    public void LoadTrajectories()
    {
        ...
        if (File.Exists(fileTrayectoria))
        {
            ...
            string[] lines = File.ReadAllLines(fileTrayectoria);
    ...
    public IEnumerator SendSelectedPointprograma()
    {
        ...
        {
            TMP_DropdownManager.DropdownOption selectedOption = dropdownManager.options[i];
            float J1 = selectedOption.value1;
            float J2 = selectedOption.value2;
            float J3 = selectedOption.value3;
            float J4 = selectedOption.value4;
