...
public class TMP_DropdownManager : MonoBehaviour
{
    ...
    public Comunicacion_irb140 comunicacionesScript;

    public void Start()
    {
        comunicacionesScript = Comunicacion.GetComponent<Comunicacion_irb140>();
    }
   ...
   public void RefreshDropdownOptions()
    ...
    {
        Debug.Log("Nombre de la opción seleccionada: " + change.value);
    ...
    {
        int selectedIndex = dropdown.value;
        Debug.Log("Nombre de la opción seleccionada: " + dropdown.value);
        if (selectedIndex >= 0 && selectedIndex < options.Count)
        {
    ...
    {
        float duracionRotacion=velocidad; // Duración de la rotación gradual
